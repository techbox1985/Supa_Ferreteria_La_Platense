...
export const CustomerStatementModal: React.FC<CustomerStatementModalProps> = ({ isOpen, onClose, customer, allSales, isAdmin, refreshData }) => {
  ...
  const summary = useMemo(() => {
    ...
    // El saldo final es la diferencia entre el debe y el haber.
            ...
            {isLoading ? (
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                    ...
                    <tr key={tx.id}>
                        ...
                        <td className="px-4 py-2 whitespace-nowrap text-sm">{tx.date ? new Date(tx.date).toLocaleString('es-AR') : 'Fecha Inválida'}</td>
                        ...
                        <td className="px-4 py-2 whitespace-nowrap text-sm text-right text-red-600">{tx.debit > 0 ? formatCurrency(tx.debit) : '-'}</td>
                        <td className="px-4 py-2 whitespace-nowrap text-sm text-right text-green-600">{tx.credit > 0 ? formatCurrency(tx.credit) : '-'}</td>
                        <td className="px-4 py-2 whitespace-nowrap text-sm text-right font-medium">{formatCurrency(tx.balance)}</td>
                    </tr>
                    ))}
                </tbody>
                </table>
