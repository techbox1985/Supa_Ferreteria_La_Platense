...
const postToScript = async (action: string, payload: any, options: { allowQueue?: boolean } = { allowQueue: true }) => {
    ...
    if (!WHITELIST_READ_ONLY.includes(action)) {
        throw new Error(`Módulo [${action}] no migrado. Escritura en Legacy bloqueada por seguridad en este repositorio.`);
    }
...
const gvizFetch = async (query: string, sheetName: string): Promise<any[]> => {
    ...
    if (!res.ok) throw new Error(`GViz fetch failed with status ${res.status}`);
    const text = await res.text();
    const match = text.match(/google\.visualization\.Query\.setResponse\((.*)\)/);
    if (!match || !match[1]) throw new Error('Invalid GViz response format.');
    const json = JSON.parse(match[1]);
    if (json.status === 'error') throw new Error(json.errors.map((e: any) => e.detailed_message).join(', '));
    const data: any[] = [];
        ...
        json.table.rows.forEach((row: any) => {
            ...
            json.table.cols.forEach((col: any, index: number) => {
                ...
                if (cell === null || cell === undefined || cell.v === null) {
                    ...
                    if (cell.f) item[`${key}_raw`] = cell.f;
                } else {
